...
export function CarouselNavigation({ isVisible = true }: CarouselNavigationProps) {
  ...
  return (
    <div className={`fixed ${isMobile ? 'bottom-8 left-1/2 transform -translate-x-1/2' : 'bottom-8 left-1/2 transform -translate-x-1/2'} z-50`}>
      <AnimatePresence>
        {isVisible && (
          <motion.div
            className={`bg-black/90 backdrop-blur-xl rounded-full ${isMobile ? 'p-2 carousel-navigation-mobile' : 'p-3'} flex items-center justify-center ${isMobile ? 'gap-1' : 'gap-1'} shadow-2xl border border-white/20 relative`}
